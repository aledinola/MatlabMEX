function [q,bp,vp,def,totaltime,avgtime] = solver_vec_gpu(b,z,m,pdf,para)
...
vp = gpuArray.zeros(ns,nb);  %continue repaying
vd = gpuArray.zeros(ns,1); 

vo = vd;
vp1 = vp;  
vd1 = vd;
def = gpuArray.false(ns,nb);

bp = gpuArray.zeros(ns,nb); %debt policy function (expressed in indices)  
bp1 = bp;
q = gpuArray.ones(ns,nb)/(1+rstar); %q is price of debt; it is a function of  (y_t, d_{t+1}) 
