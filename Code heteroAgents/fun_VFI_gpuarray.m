function [V,Policy,Params] = fun_VFI_gpuarray(p_eqm, a_grid, z_grid, pi_z, Params, vfoptions)
...
ReturnMatrix = - Inf(size(cons),'like',cons);
mask = cons>0;
...
V0     = zeros(n_a,n_z,'gpuArray') ; % Initial guess V0
...
iAvec = repmat(gpuArray.colon(1,n_a).',n_z,1);    % n_a*n_z × 1
iZvec = repelem(gpuArray.colon(1,n_z).', n_a, 1); % n_a*n_z × 1
